...
export default function CropDetailsScreen() {
  ...
  const { crop, editMode } = route.params || {};
  ...
  const [formData, setFormData] = useState({
    id: editMode ? crop.id : null,
    cropName: editMode ? crop.cropName : "",
    cropType: editMode ? crop.cropType : "",
    area: editMode ? extractValue(crop.area) : "",
    areaUnit: editMode ? extractUnit(crop.area, "acre") : "acre",
    sowingDate: editMode ? parseDate(crop.sowingDate) : new Date(),
    expectedFirstHarvest: editMode ? parseDate(crop.expectedHarvestStart) : new Date(Date.now() + 90 * 24 * 60 * 60 * 1000),
    expectedLastHarvest: editMode ? parseDate(crop.expectedHarvestEnd) : new Date(Date.now() + 120 * 24 * 60 * 60 * 1000),
  ...
  // FIXED: Simplified date picker state
  ...
  // Format date to DD-MM-YYYY for storage (matching view screen format)
  ...
  // FIXED: Improved date picker handler
  const openDatePicker = (field) => {
  ...
  const handleDateChange = (event, selectedDate) => {
    // Close the picker on both Android and iOS
    ...
    if (selectedDate && currentDateField) {
      ...
      // For iOS, we need to close after setting the value
      if (Platform.OS === 'ios') {
  ...
  // In your CropDetailsScreen
const handleSaveCrop = (cropData) => {
  // Call the addCrop function passed via navigation params
  route.params?.onCropAdd?.(cropData);
  navigation.goBack();
};
  ...
  if (!formData.expectedYield.trim()) {
  ...
  const submissionData = {
    ...
    expectedYield: `${formData.expectedYield} ${formData.yieldUnit}`,
    sowingDate: formatDateForStorage(formData.sowingDate),
    expectedHarvestStart: formatDateForStorage(formData.expectedFirstHarvest),
    expectedHarvestEnd: formatDateForStorage(formData.expectedLastHarvest),
    // For display in MyCropsScreen
    plantedDate: formatDateForStorage(formData.sowingDate),
  };

  if (editMode) {
    // Update existing crop
    console.log('Updating crop:', submissionData);
    
    // Call the update callback if provided
    if (route.params?.onCropUpdate) {
      route.params.onCropUpdate(submissionData);
    }
    ...
    // Create new crop
    console.log('Creating new crop:', submissionData);
    
    // Call the add callback if provided
    if (route.params?.onCropAdd) {
      route.params.onCropAdd(submissionData);
    }
      ...
      <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
        ...
        <View className="mx-4 mt-6 mb-6">
          ...
          >
            ...
            <Text className="text-white font-bold text-lg ml-2">
              {editMode ? "Save Changes" : "Send for Verification"}
            </Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
